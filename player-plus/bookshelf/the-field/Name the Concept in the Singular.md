...
pillar: —
---
# Name the Concept in the Singular

*A concept page is named in the singular — because the page at a name is the meaning of every link that points to it, and the singular is the concept itself; the plural is only many of it.*

[[Cell]]. [[Quest]]. [[Story]]. [[Flow]]. Not *Cells*, *Quests*, *Stories*. The reason is not tidiness but meaning: every link is a claim about what a word means, and the place it lands is where that meaning lives. A plural name quietly says *a group of these* where the concept was wanted, and a body that has both ends up with two homes for one idea and a slow divergence between them ([[One canonical home per concept]] · [[Link concepts, not coordinates]] · [[Semantic Capture]]).

The plural is written by piping — `[[Cell|Cells]]` — so the sentence reads naturally and the link still lands on the concept. That single move is what lets prose stay human while the graph stays clean.
