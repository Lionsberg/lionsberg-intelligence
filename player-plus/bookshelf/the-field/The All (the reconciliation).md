...
The resolution offered is a single test, and it is a test of direction rather than of scope: *is our work with the few entirely in service of All?* Not partly. Not eventually. The few are attended to because that is how finite beings actually reach anything, and the reaching is aimed outward from the start. Held that way, concentration is a method and never a boundary ([[The Goal]], [[Throughput]], [[Co-Creating Heaven]]).

The test is usable, which is the point of stating it so plainly. It fails the moment a body's advantage becomes the reason for its focus rather than a by-product of it; when what is learned in the few stops travelling outward ([[Knowledge as commons]], [[Share the Stories]]); when the boundary that made the work possible becomes the thing being defended ([[Selectively Permeable Membrane]], [[Guard the Membrane]], [[Ideological Capture]]). Each of those is the same failure at a different scale, and each is caught by the same question ([[The compass question]]).

**Also called:** The All · the few in service of All
