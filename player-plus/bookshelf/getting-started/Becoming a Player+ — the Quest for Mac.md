...
4. If it asks to "find devices on your local network," choose **Do not Allow**.
...
3. Wait. It can take five to forty minutes; the first estimate is often wildly wrong. It can look idle; do not stop it.
...
5. Paste it into iTerm. Press Return, and wait. For a minute or two it may look idle; it is working. Do not stop it.
...
3. Open the kit's **player-plus** folder. Press Cmd+Shift+. (the full stop) so hidden items show; a faint `.claude` folder appears, holding the kit's two permissions for the Dojo. Copy everything inside **player-plus**, `.claude` too, into **HQ**.
...
6. When the browser says **You are all set up**, return to iTerm and press Return.
