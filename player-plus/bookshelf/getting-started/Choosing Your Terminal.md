...
The terminal is where you will actually talk to Claude Code — it is the window where you type and where your agent replies. You will spend a lot of time looking at it, so it is worth two minutes now to make sure it is set up well.

The built-in terminal on your computer works, but on both Mac and Windows there is a better option, and on Windows there is a real gotcha to fix before you go further. Follow the section for your platform.

---

## Mac

The Mac comes with a Terminal app already installed, and it is fine to start with. But we recommend **iTerm2** instead — it is free, and a small upgrade in day-to-day comfort.
...
> [!tip]
> The concrete win with iTerm2: selecting text with your mouse copies it automatically (no need to Cmd+C), and it gives you much more control over appearance — colors, transparency, font size — than the built-in Terminal app. Small thing, but you will notice it every day.
...
> [!warning]
> Do not use Command Prompt, and do not use Git Bash as your everyday terminal. Windows Terminal running PowerShell is what this kit assumes.
...
Windows Terminal opens with PowerShell as the default tab, so you are usually already there: the tab says **PowerShell**. If not, click the small down-arrow next to the `+` at the top of the window and choose **Windows PowerShell** from the list.
...
Windows Terminal's default color scheme (often called "Tango Light" or similar) is a **light background**. Claude Code prints some text in **light colors**, designed for a **dark background**. Put the two together and you get near-white text on a near-white background — text that is technically there but that you simply cannot see.

This shows up as things like a numbered list that appears to have gaps in it, or highlighted text that looks blank. It is not a bug in Claude Code, and it is not your imagination — it is a color scheme mismatch, and it is a two-minute fix.
...
2. Choose **Settings**.
3. Click **Color schemes** in the sidebar — you will see a paint-box grid of color scheme options.
...
Once Claude Code is running and you have made both changes, ask it to print a numbered list with a few words in bold or highlighted. If every item is clearly visible and readable, you are set. If anything looks blank, missing, or washed-out, stop here and redo the color scheme steps above before going further — everything downstream in this kit assumes you can actually read what your agent sends you.

---

## A pop-up about your local network

If a pop-up asks to let the terminal "find devices on your local network," choose **Do not Allow**.
