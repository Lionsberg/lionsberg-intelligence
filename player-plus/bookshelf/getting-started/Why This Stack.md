...
You are about to set up a small system of tools that work together. Here's what each one does and why they fit together.
...
The terminal is where you talk to it. That is the whole interface — no app to open, no sidebar to configure. You type what you want, in plain English, and your agent does it.

## Markdown Files — Where Everything Lives

Everything your agent works with — its own memory, your notes, your projects — is stored as plain text files (Markdown) on your own hard drive. Unlike Google Docs or Notion, none of it is locked in someone else's cloud. You can open these files with any text editor, move them anywhere, back them up however you like, and they will still be perfectly readable in twenty years — Markdown is just text with a few simple conventions (`#` for a heading, `**bold**` for bold), not a proprietary format.
...
Git is a version-control tool used by software teams worldwide. It keeps a complete history of every change you make — like an infinite undo button that remembers every version of every file. GitHub is a website that stores a copy of your files in the cloud, so they are backed up and, if you choose, visible to collaborators.

Here's the part that is different in this kit: **your agent handles git for you, silently.** You do not memorize commands, and you do not get asked about commits — your agent just keeps a careful history running in the background as you work with it. Git only needs to be installed, which the Quest does at Gate 4. If you are curious how it works under the hood, or want to add GitHub for cloud backup, that is covered in the optional sections later in this book — but GitHub is never required.
...
Put together, this is a system where you own your files outright, your agent works directly on them at your direction, and every change is automatically checkpointed into a recoverable history. The result is private (your files live on your own computer; what your agent reads to help you is sent to Anthropic to be processed, and nothing else leaves unless you choose), durable (plain text you can still read in twenty years), auditable (every change is a readable record, not a black box), and it gets more useful the longer you use it — because your agent's memory and your own body of notes both keep growing.
