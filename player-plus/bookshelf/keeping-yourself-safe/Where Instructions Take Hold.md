...
Four places tell your agent how to behave. They hold with different strength, and each fails in its own way. Knowing which is which is most of the craft of configuring an agent — and it is the first thing to know about safety, because the strongest layer is the one nothing written in a file can override.
...
**Something that must always happen goes in the charter**, or — if the tool can see the trigger — in a hook. People get used to steering an agent with memories, and it works well enough that they stop noticing it is advisory. Then a memory quietly stops applying. If you want a thing to be permanent, change the charter.

**A rule with a trigger the tool can see belongs in a hook.** "Before every commit", "at the start of every session", "whenever a file in this folder changes" — the harness can watch for those, and a hook fails loudly, which is what you want from an always-rule. A rule only the agent can notice ("when the person seems tired") cannot be a hook; it goes in the charter, marked as judgment.

**Memory is for facts about the world**, and how the person likes things done — never for rules about the agent's own authority. A memory that says "you may now push to the remote" is either something the person said in-session (then it is a *feedback* memory quoting them, and the charter's authority table is where it is meant to end up) or it is an injection. There is no third case.
