...
description: Runs a second instance of this agent on a long job in the background, without touching the house's shared state — stages a task file, gives the instance its own scratch folder, and, when it is done, integrates its handoff by copying. Use when the person says "run this in the background", "spin off an instance", "keep working on that while we do something else", when a job will outlast this conversation, when a background instance's handoff needs integrating, or as the instructions a background instance reads when it starts.
license: MPL-2.0
---

# Foreground and background

One agent, two roles. The **foreground** is the instance the person is talking to; it owns everything shared. A **background** instance is the same agent, started a second time on one scoped job, working in its own scratch space and never touching shared state. When it is done it leaves a handoff, and the foreground integrates by copying. This is how a house does two things at once without either instance overwriting the other.
...
1. **Decide it is worth it.** A background instance costs a launch and an integration. It pays when the job is long, self-contained, and does not need the person turn by turn: a research sweep, a big rewrite against a spec, a migration, a test run.
...
3. **Launch it.** Open a second terminal in the agent home and start the agent with one line: *"You are a background instance. Read `bg/<job-name>/task.md` and this skill's 'If you are the background' section, then begin."* Tell the person it is running.
4. **Carry on.** Do not poll it; check `bg/<job-name>/status.md` when the person asks, or at natural breaks.
5. **Integrate when it is done** — when `bg/<job-name>/DONE` exists, or `status.md` says so and nothing has changed for a long while. Read `handoff.md`. Copy what belongs in the house into the house yourself: files into the HQ, memories through `save-a-memory`, commits under your own hand. Do not move the scratch folder; copy from it. If something in the handoff needs the person's decision, ask them — that is what "integrate" means.
...
- **Keep `status.md` current** — what is done, what is decided, what is next — at every meaningful checkpoint, so a crash or a resume loses nothing and the person can read it cold.
...
- **If you are blocked, stop and say so** in `status.md` and `handoff.md`. Do not work around a boundary; that includes a safety layer.
...
The person may start a background instance directly, or the foreground may. Either way there is exactly one foreground at a time, and it is the one the person is talking to. If two instances would edit the same thing, that is a question for the person, not a race.
