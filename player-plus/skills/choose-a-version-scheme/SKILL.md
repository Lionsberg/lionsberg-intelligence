...
4. **Versions coexist.** A new major version sits beside the old one; the old one keeps its number, its tag, its docs. Do not rewrite a dated artifact to a newer version; it was that version on its day.
5. **Bump when the thing changes for its readers, not when you feel like it.** A release with nothing in CHANGES is not a release.

## Do not pin the future to a number
