...
- Confirm the person actually wants this now, and roughly how much of the history — everything, or a slice. This is their record; do not assume "all of it" is the right first move.
- Ask where the export will land on disk before you touch anything. You will want a location with enough room — these exports can run from tens of megabytes to well over a gigabyte, and they are not something to commit into a git repository whole.
- Check the current export format before you build anything against it. OpenAI's export shape has changed before and will change again — confirm what is actually in the zip you have rather than assuming last year's structure holds.
...
- ChatGPT's export has not, so far, included "Projects" (or equivalent custom-workspace content) — check whether that is still true for the export you are holding, and tell the person plainly if something they expected is not there rather than silently omitting it.
- Get the date range and rough conversation count from the export metadata before you process anything, and report it back to the person. It is a useful sanity check later — if the processed count does not match, something broke.
- A single export can span years and thousands of conversations, but most of it tends to be short exchanges rather than a handful of long threads. Do not assume you are walking into a few deep conversations to read closely — plan for many small ones instead.

## Where it goes in the house

- Give the export its own place under the house, kept separate from the person's hand-curated notes. Think of it as boxes beside the book: the raw, unzipped export is the sealed boxes — kept intact, gitignored if the house is version-controlled, never edited in place — and the processed markdown is the book the person and you actually read and search. Do not mix machine-made notes into hand-curated space; if something distilled from the archive earns a permanent home there later, promote just that piece, deliberately.
- Do not commit the raw export itself to a git history. It is large, it is personal, and it does not need version control — the processed form is what evolves.
- If the person exports again later — a fresh pull months on — treat it as a new, non-overlapping slice rather than assuming it is a clean superset of the last one; check before you decide whether the new export replaces or supplements what is already in the house.

## Processing: one file per conversation

- Convert the export into one markdown file per conversation, each with frontmatter carrying at minimum the conversation's date and title, organized into a sensible folder structure (by month works well for a multi-year archive). This step is purely mechanical — no model calls needed — and should be idempotent, so re-running it after a fresher export does not duplicate or corrupt anything already converted.
...
- Watch for two format gotchas that show up in real exports and are easy to miss:
  - Markdown pulled from a chat UI can carry invisible private-use control characters embedded around structured UI blocks. They will not match an obvious text search for the visible word next to them; filtering by Unicode private-use codepoint ranges catches them.
...
- Do not skip straight from "converted" to "the agent now knows this." Distillation — reading conversations and writing durable facts about the person — is a separate, deliberate step, and it needs the person's explicit go-ahead before it happens, not just before it is promoted.
...
- For each conversation (or batch), extract only what is useful: a short summary, topic tags, named entities, and candidate durable facts about the person — each fact linked back to the conversation it came from. That backlink matters: a fact without its source is a rumor.
...
- Validate your extraction approach on one small batch before running it over the whole corpus — a bad prompt caught early is a rewrite; caught late, it is a corpus-wide cleanup.
...
- Never treats "the export exists" as consent to distill it into standing memory — that is a separate ask.
...
- Never invents a fact about the person that is not backed by a specific line in the source conversation.
