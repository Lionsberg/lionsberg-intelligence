...
description: Puts a document in front of the person on the surface they can actually read it — opens it in their review app on the computer, or puts it on a shared pad and gives the link when they are on a phone — commits the draft first if the house keeps git so their edits show as a clean diff, and reads their changes back afterward. Use when the person says "let me see it", "open that", "send it to me", "put it where I can read it", "I'm on my phone", when you have written more than a screen of markdown, or when a draft needs the person's markup before anything else happens.
license: MPL-2.0
---

# Send for review

Work in documents, not chat. A draft that scrolls past in the terminal is not reviewed; a draft the person opens in their own review surface, marks up, and hands back is. This skill is the hand-off, and the read-back.

## Which surface

Ask once, then remember (`save-a-memory`): what does the person read markdown in? On a computer: Typora, MarkText, Obsidian, VS Code, whatever they already use. On a phone: a shared pad — MeetingWords or its like — because a phone cannot open a file on the computer. The kit's *Viewing Your Files* chapter is the list if they have nothing yet.
...
3. **Say what you'd like from them**, in one sentence: "mark anything up; I'll read the diff." Then wait. Do not narrate while they read.
...
5. **Commit the reviewed version**, message noting it is post-review. Remove any `.before` copy.

## Small courtesies that matter

- Their edits carry weight: do not "fix" a change back to your wording.
- If they changed a fact you know to be wrong, say so once, plainly, and let them decide.
- On a phone, keep everything else short while they are reading; the reply thread is their only window.
